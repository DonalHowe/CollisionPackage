...
public class collisionComponentTests
{
    ...
    public void aabbTest()
    {
        ...
        BoxCollider2D leftCollider = LefttSquare.GetComponent<BoxCollider2D>();
    ...
    [Test]
    public void CircleToCircleTest()
    {
        // creating the objects 
        GameObject rightCircle = MonoBehaviour.Instantiate(Resources.Load<GameObject>("Prefabs/circleTwo"));
        GameObject LeftCircle = MonoBehaviour.Instantiate(Resources.Load<GameObject>("Prefabs/circleOne"));
        GameObject colManager = MonoBehaviour.Instantiate(Resources.Load<GameObject>("Prefabs/CollisionManager"));


        Rigidbody2D RightmovmentRb = rightCircle.GetComponent<Rigidbody2D>();
        Rigidbody2D LeftmovmentRb = LeftCircle.GetComponent<Rigidbody2D>();

        // creating the collider
        CircleCollider2D RightCollider = rightCircle.GetComponent<CircleCollider2D>();
        CircleCollider2D leftCollider = LeftCircle.GetComponent<CircleCollider2D>();

        colManager.GetComponent<circleTocircle>().VarInitFunc(RightCollider, leftCollider);
      
        rightCircle.transform.position = new Vector3(0, 0, 0);
        LeftCircle.transform.position = new Vector3(0.1f, 0, 0);

        bool status = colManager.GetComponent<circleTocircle>().circleToCircleFunc();
